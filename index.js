   ...
   switch (prefix) {
      ...
      case 'info':
       ...
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '[OXENTE RAPARIGA ESPERE...', MessageType.text)
       ...
       case 'ytmp3':
           ytdl('mp3', value)
               .then(data => {
                   ...
                   let hasil = `OPA PAINHO AQUI ESTA DA MSC\nCLIQUE NO LINK ABAIXO:\nMUSICA: ${judul}\n\nTamanho Do audio: ${size}\n\nLink: ${link}`
       ...
       case 'ytmp4':
           ytdl('mp4', value)
               .then(data => {
                   const { judul, size, hasil: link } = data
                   let hasil = `OPA PAINHO ESTA O LINK DE DOWNLOAD DA MSC\nCLIQUE NO LINK ABAIXO:\nMUSICA: ${judul}\n\nTamanho Do audio: ${size}\n\nLink: ${link}`
                   client.sendMessage(id, '[OXENTE RAPARIGA SE AQUETE...', MessageType.text)
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
   ...
   case 'fb':
       ...
       case 'ocr':
           ...
           readTextInImage(media)
               .then(data => {
                   client.sendMessage(id, `*ola meu rei o texto da imagem esta aqui.* \n\nResultado:: \n\n${data}`, MessageType.text);
               })
               .catch(err => {
                   console.log(err)
               })
  ...
  case 'cektanggal':
   ...
   case 'bitly':
           bitly(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui maninha pronto meu bom☣️ :) \n\n${result} `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
